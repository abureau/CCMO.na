...
$$\mbox{pr}(G^m=k|X)=\frac{\xi_k(\theta)e^{k\eta'X_{G^m}}}{\sum_l\xi_l(\theta)e^{l\eta'X_{G^m}}},$$
where $\eta$ is a regression parameter vector,$X_{G^m}$ is a function of $X$, $\xi_k(\theta)$ is the probability of $G^m=k$ with $\theta$ being the minor allele frequency. 
...
Xgm <- X
f <- 0.01
fit <- CCMO.na(Y,gm,gc,Xo,Xm,Xc,Xgm,f)
```
